...
| صفحه | فایل | توضیح |
|------|------|--------|
| پوسته | `views/AppShell.vue` | چیدمان سه‌ستونه، کشوهای موبایل، میزبان مودال ایجاد پست و جست‌وجو |
...
جست‌وجو (`SearchModal.vue`، بازشونده از ورودی/زبانه‌های سایدبار یا نوارهای بالا/پایین موبایل): پنجره‌ای ثابت روی مرکز صفحه با همان سه زبانه — **username** (`GET /api/users?query=`، حداکثر ۸ نتیجه، کلیک ← پروفایل)، **post** (`GET /api/posts/search?query=`، ردیف‌های کوچک، کلیک ← جزئیات پست)، و **general** که هر دو را ترکیب می‌کند با میان‌برهای «دیدن همه». مودال به `document.body` تلپورت می‌شود تا اسکرول‌بار/حاشیه کشوی سایدبار هرگز روی آن نیاید؛ در موبایل همیشه تکی باز می‌شود — `AppShell.vue` کشوها را می‌بندد به‌جای این‌که کشو پشت آن بماند (جست‌وجوی شروع‌شده از سایدبار هم با رویدادهای `search-opened`/`search-closed` از طریق `LeftSidebar.vue` همین رفتار را می‌گیرد). دیباونس ۳۰۰ میلی‌ثانیه، بستن با `Escape`/پس‌زمینه/دکمه بستن، آواتار برای ردیف‌های کاربری.
