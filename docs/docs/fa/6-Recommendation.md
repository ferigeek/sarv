...
> **نکته قرارداد:** همین `GET /feed` رتبه‌بند UC-12 «دریافت فید هوشمند» است — عمداً مسیر جداگانه `/smart_feed` وجود ندارد. کاربران `GET /api/feed/recommended` بک‌اند را صدا می‌زنند که رتبه‌بندی را به اینجا واگذار می‌کند.
