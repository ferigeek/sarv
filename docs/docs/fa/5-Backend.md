...
همه نقاط پایانی با پیشوند `/api` شروع می‌شوند. به جز مواردی که با **عمومی** مشخص شده‌اند، هر نقطه پایانی به هدر `Authorization: Bearer <token>` نیاز دارد و در صورت نبود یا نامعتبر بودن JWT پاسخ `401` برمی‌گرداند. مصرف‌کننده اصلی وب‌کلاینت فرانت‌اند است (به [7-Frontend.md](./7-Frontend.md) مراجعه کنید) که این اندپوینت‌ها را به‌صورت هم‌مبدأ از طریق پراکسی `/api` خود صدا می‌زند.
...
| GET | `/api/users/{userId}/reacted-posts?filter=` | bearer | پست‌های صفحه‌بندی‌شده‌ای که کاربر به آن‌ها واکنش نشان داده، از جدیدترین واکنش؛ `filter` برابر `ALL` (پیش‌فرض)، `LIKE` یا `DISLIKE` است؛ پارامتر `sort` کلاینت نادیده گرفته می‌شود؛ پست‌های حذف نرم‌شده کنار گذاشته می‌شوند |
| GET | `/api/users/{userId}/stats` | bearer | تعداد دنبال‌کننده‌ها و دنبال‌شونده‌ها (`{userId, followerCount, followingCount}`)، جداگانه از پروفایل سرو می‌شود تا محاسبه شمارش‌ها هیچ‌وقت نمایش پروفایل را متوقف نکند |
...
ترکیب‌های نامعتبر با `400 Bad Request` (`PostNotValidException`) رد می‌شوند. ایجاد `COMMENT` به‌صورت اتمیک `comment_count` پست والد را افزایش می‌دهد. به‌روزرسانی پست از معنای `PUT` استفاده می‌کند تا فیلد با `null` صریحاً پاک شود. حذف پست `deleted_at` را تنظیم و ارجاع نویسنده را پاک می‌کند (حذف نرم).
...
- شمارش بازدید — هر پستی که از هر یک از دو نقطه پایانی فید سرو شود، یک بازدید می‌گیرد (افزایش اتمیک دسته‌ای با `incrementViewCounts`) و `PostResponse` برگردانده‌شده از قبل آن را نشان می‌دهد؛ پست‌های حذف‌شده/یافت‌نشده هیدراته از افزایش کنار گذاشته می‌شوند.
...
آپلودها با SHA-256 آدرس‌دهی محتوا می‌شوند؛ بنابراین محتوای یکسان فقط یک بار ذخیره می‌شود. آپلود مجدد بایت‌هایی که قبلاً وجود دارند، به‌جای خطا همان `{id, url}` موجود را با `201 Created` برمی‌گرداند — ردیف `Media` تکراری ساخته نمی‌شود (در حالت مسابقه هم‌زمان، به ردیف برنده ارجاع داده می‌شود).
...
نقاط پایانی فهرستی، اشیای `Page` اسپرینگ دیتا را با `page`, `size`, `totalElements` و `totalPages` برمی‌گردانند. اندازه پیش‌فرض صفحه ۲۰ است؛ مرتب‌سازی پیش‌فرض: جست‌وجوی کاربران بر اساس `username`، پست‌های کاربر و جست‌وجوی پست بر اساس `createdAt DESC`، کامنت‌های پست با `sortBy` سمت سرور (`NEWEST`/`MOST_LIKED`)، دنبال‌کننده‌ها بر اساس `follower.username`، دنبال‌شونده‌ها بر اساس `followed.username`، فید زمانی بر اساس `createdAt DESC`؛ پست‌های واکنش‌نشان‌داده‌شده و فید پیشنهادی **بدون مرتب‌سازی** هستند (به ترتیبِ تاریخ واکنش از جدید به قدیم و `score desc` سمت سرور، هر `sort` کلاینت نادیده گرفته می‌شود). کلاینت‌ها می‌توانند با پارامترهای استاندارد `page`, `size`, `sort` آن را تغییر دهند، مگر جایی که ترتیب سمت سرور تعیین می‌شود؛ recommended مقدار `page/size` را به سرویس توصیه‌گر (`size` ۱ تا ۱۰۰) ارسال کرده و `total` آن را برای فراداده صفحه استفاده می‌کند.
