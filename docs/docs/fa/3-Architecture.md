...
graph TD
    ...
    Backend -. HTTP REST - برنامه‌ریزی‌شده .-> Recommender
...
> **توجه:** یکپارچه‌سازی بین هسته مرکزی و سرویس Recommendation طراحی شده است ولی هنوز پیاده‌سازی نشده است. نمودار توالی زیر جریان برنامه‌ریزی‌شده فید هوشمند را نشان می‌دهد؛ در حال حاضر هیچ نقطه پایانی فید وجود ندارد (به بخش «وضعیت پیاده‌سازی» مراجعه کنید).
...
در زمان دریافت فید، طبق طراحی هسته مرکزی داده‌های اولیه را از پایگاه داده دریافت کرده و در صورت نیاز برای رتبه‌بندی به زیرسیستم Recommendation ارسال می‌کند. این جریان برنامه‌ریزی‌شده است؛ به بخش «وضعیت پیاده‌سازی» مراجعه کنید.
...
این بخش به صورت یک سرویس مستقل در Python (FastAPI) پیاده‌سازی شده است و هیچ مسئولیتی در زمینه مدیریت کاربران یا ذخیره‌سازی داده‌ها ندارد. در پیاده‌سازی فعلی، این سرویس کاندیدها را از بک‌اند دریافت نمی‌کند؛ بلکه پست‌های کاندید را مستقیماً از پایگاه داده مشترک PostgreSQL می‌خواند:

- **پست‌های داغ (Trending):** پست‌های اخیر (۷ روز گذشته) با تعامل بالا، مرتب‌شده بر اساس میزان تعامل
- **پست‌های دنبال‌شونده‌ها:** پست‌های اخیر کاربرانی که کاربر درخواست‌دهنده دنبال می‌کند
- **پست‌های دنبال‌کننده‌ها:** پست‌های اخیر کاربرانی که کاربر درخواست‌دهنده را دنبال می‌کنند

کاندیدها حذف تکراری شده، امتیازدهی و بر اساس امتیاز نزولی مرتب می‌شوند و از طریق `GET /feed` بازگردانده می‌شوند. هر آیتم شامل `post_id` و `score` است؛ امتیاز از تعامل (لایک + بازدید − دیسلایک)، پوسیدگی زمانی (نیمه‌عمر حدود ۴۸ ساعت) و ضریب تقویت برای پست‌های کاربران دنبال‌شده محاسبه می‌شود.

> **نکته درباره قرارداد ارتباطی:** این سرویس در حال حاضر هم `post_id` و هم `score` را برمی‌گرداند. قرارداد نهایی این است که سرویس پیشنهاددهی **فقط شناسه پست‌ها** را به هسته مرکزی بازگرداند و هسته مرکزی داده کامل پست‌ها را از پایگاه داده هیدراته کند.

یکپارچه‌سازی با هسته مرکزی هنوز پیاده‌سازی نشده است: بک‌اند در زمان تولید فید این سرویس را فراخوانی نمی‌کند. طبق طراحی، هسته مرکزی مجموعه‌ای از پست‌های کاندید را انتخاب کرده و همراه با اطلاعات مورد نیاز کاربر به این زیرسیستم ارسال می‌کند، شناسه‌های رتبه‌بندی‌شده را دریافت و فید نهایی را هیدراته می‌کند. در صورت از دسترس خارج شدن این سرویس، هسته مرکزی قادر خواهد بود بدون توقف سیستم، از فید زمانی ساده استفاده کند و سرویس‌دهی را ادامه دهد (تخریب مهربانانه).
...
| ثبت رویداد (`event_logs`) | پیاده‌سازی‌شده |
| تولید فید (زمانی / هوشمند) | برنامه‌ریزی‌شده — هنوز نقطه پایانی فید وجود ندارد |
| یکپارچه‌سازی بک‌اند با Recommendation | برنامه‌ریزی‌شده — سرویس به‌صورت مستقل اجرا می‌شود، مستقیماً از PostgreSQL می‌خواند و `post_id` + `score` برمی‌گرداند |
