...
مانیتورینگ بخشی از مسیر پردازش درخواست‌های کاربران نیست، اما نقش مهمی در نگهداری و مدیریت سیستم دارد. این زیرسیستم **با docker compose مستقر می‌شود** — بک‌اند نقاط Actuator شامل `health,metrics,prometheus` را اکسپوز می‌کند (مسیر عمومی `/actuator/**`، تگ `application=sarv`، هیستوگرام پرسنتیل فعال)، Prometheus بک‌اند و جاب‌های Postgres/Redis/توصیه‌گر را اسکرپ می‌کند (`monitoring/prometheus.yml`) و Grafana داشبوردهای آماده دارد (`monitoring/grafana/`).
...
| ثبت رویداد (`event_logs`) | پیاده‌سازی‌شده — ۱۳ نوع رویداد از جمله `REGISTER` و `QUOTE_POST`؛ `REQUEST_FEED` با `metadata {feed_type}`؛ گزارش dwell پست با `POST /api/posts/{id}/dwell` (رویداد `VIEW_POST` با `metadata {duration_ms, source}`) با گروه‌بندی `session_id` مختص هر زبانه (ایندکس V8) |
...
| مانیتورینگ (Prometheus / Grafana) | پیاده‌سازی‌شده — نقطه `prometheus` اکچوئیتور اسکرپ می‌شود؛ داشبوردها آماده‌اند |
| Redis (کش و محدودسازی نرخ) | اعلام‌شده و در حال اجرا (اکسپورتر اسکرپ می‌شود)، بدون استفاده در کد برنامه |
